...
{
public:
	CUiObj(LAYER_TYPE Layer = LAYER_TYPE::LAYER_05);	// デフォルトコンストラクタ
	~CUiObj();	// デフォルトデストラクタ
	...
	void SetSelect(bool select) { m_bSelect = select; }	// 選んでるフラグの設定
	string GetType(void) { return m_TexType; }	// 種類の取得
	bool GetSelect(void) { return m_bSelect; }	// 選んでるフラグの取得
