...
// 静的メンバ変数宣言
//=============================================================================
CModel::MODEL_DATA *CModel::m_aModelData;
...
HRESULT CModel::Init(void)
{
	...
	int nTexNum = CTexture::GetTexNum();	// テクスチャ総数
...
HRESULT CModel::Init(int *nPreants, FILE *pFile)
{
	...
	if (pFile != NULL)
	{
		...
		while (true)
		{
			...
			//END_PARTSの文字列を見つけたら
			else if (strcmp(&aFile[0], "END_PARTS") == 0)
			{
				// ループ抜け
				break;
			}
...
HRESULT CModel::Load(void)
{
	...
	int nModelMax;	// モデル最大数
	int nPartsNum = 0;	// パーツ数
	char aPas[MAX_CHARDATA][MAX_CHARDATA];	// パス
	char aFile[MAX_CHARDATA];	// ファイルの文字列バッファ
	...
	if (pFile != NULL)
	{
		...
		while (true)
		{
			...
			// MODEL_FILENAMEの文字列を見つけたら
			else if (strcmp(&aFile[0], "MODEL_FILENAME") == 0)
			{
				fscanf(pFile, "%s", &aFile[0]);
