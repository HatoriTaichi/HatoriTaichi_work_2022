	...
	for (int count_y = 0; count_y < TROPHY_Y_LINE; count_y++)
	{
		...
		for (int count_x = 0; count_x < TROPHY_X_LINE; count_x++)
		{
			...
			if (trophy_flag[trophy_count])
			{
				...
				switch (trophy_count)
				{
				case static_cast<int>(TROPHY::AIRPLANE):
			...
			// トロフィーカウントが最後まで行ったら
			if (trophy_count >= static_cast<int>(TROPHY::MAX))
			{
				// ぬける
				break;
			}
